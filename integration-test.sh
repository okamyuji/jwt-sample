...
wait_for_health() {
    ...
    while [[ $attempt -le $max_attempts ]]; do
        if curl -s "${BASE_URL}/actuator/health" 2>/dev/null | grep -q '"status":"UP"'; then
...
test_health_check() {
    ...
    assert_status "GET /actuator/health" "200" "$status_code"
}
...
test_refresh_token_invalid_token() {
    ...
    # 無効なトークンは400, 403, 500のいずれかのエラー
    if [[ "$status_code" =~ ^(400|403|500)$ ]]; then
        log_success "POST /api/v1/auth/refresh-token (invalid token) (HTTP $status_code)"
        ((PASSED++))
    else
        log_error "POST /api/v1/auth/refresh-token (invalid token) - Expected: 400/403/500, Got: $status_code"
        ((FAILED++))
    fi
}
